...
/*   Updated: 2021/03/26 16:44:05 by kanlee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "trace_ray.h"

static t_color sample_color(t_ray ray, t_viewport vp, double i, double j, t_mlx *rt)
{
	t_color ret;

	ray.direction = set_ray_direction(ray, vp, i / (rt->screen_height - 1), j / (rt->screen_width - 1));
...
#if 1
	t_color aa = color(0,0,0);
	aa = c_add(aa, sample_color(ray, vp, (double)i - 0.5, (double)j, rt));
	aa = c_add(aa, sample_color(ray, vp, (double)i + 0.5, (double)j, rt));
	aa = c_add(aa, sample_color(ray, vp, (double)i, (double)j - 0.5, rt));
	aa = c_add(aa, sample_color(ray, vp, (double)i, (double)j + 0.5, rt));
	set_pixel_color(rt, i, j, aa);
#endif
}
